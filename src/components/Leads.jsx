...
export default function Leads() {
  ...
  useEffect(() => {
    const fetchLeads = async () => {
      try {
        const res = await axios.get(`${API}/lead-form`, {
  ...
  return (
    <Card className="shadow-md border border-gray-200 hover:shadow-lg transition-all duration-300 lg:mt-10">
                ...
                filteredLeads.map((lead, i) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <Badge
                        variant="outline"
                        className={`capitalize border font-medium px-2.5 py-1 text-xs rounded-full ${
                          purposeColors[lead.purpose] || purposeColors.other
                        }`}
                      >
                        {lead.purpose}
                      </Badge>
